...
internals.normalizeOptions = (options) => {
    ...
    const result = Object.assign({ uri: 'mongodb://' }, options);
    const conns = options.connections;

    if (conns.constructor === Array) {
        let conn;
...
exports.register = (plugin, options, next) => {
    ...
    if (schemaValidate.error !== null) {
        plugin.log(['error', 'plugin', 'mongo', 'optionsValidation'], schemaValidate.error);
    ...
    startDb = (callback) => {
        ...
        db = mongoose.connect(uri, mongoOptions, (err) => {
            ...
            if (err) {
                ...
                plugin.log(['error', 'plugin', 'mongo', 'connection'], entry);

                return false;
            }

            plugin.log(['plugin', 'mongo', 'connection', 'start'], entry);
        });
    ...
    stopDb = (shouldKill = true, done) => {
        ...
        mongoose.disconnect(() => {
            ...
            const entry = { message: `Disconnect from: ${ uri }` };

            plugin.log(['plugin', 'mongo', 'connection', 'stop'], entry);

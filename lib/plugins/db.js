    ...
    internals.defaults.validate(options, (err, opts) => {
        ...
        startDb = () => {
            ...
            db = Mongoose.connect(url, opts.mongoOptions, (err) => {
                ...
                const { host, port, _db } = opts;

                if (err) {
                    plugin.log(['error', 'mongo', 'connection'], { desc: 'Failed to connect', host, port, db: _db });

                    return false;
                }

                plugin.log(['mongo', 'connection'], { desc: 'Successfully connected', host, port, db: _db });
            });

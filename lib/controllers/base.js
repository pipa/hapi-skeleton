...
class Base {
    ...
    create(request, reply) {
        ...
        newDoc.save((err, doc) => {
            ...
            return this.coda(request, reply, err, doc, data, ['database', 'save']);
        });
    }
    ...
    read(request, reply) {
        ...
        this.get(query, limit).exec((err, docs) => {
            ...
            return this.coda(request, reply, err, docs, data, ['database', 'find']);
        });
    }
    ...
    update(request, reply) {
        ...
        this.Collection.findOneAndUpdate(conditions, payload, { new: true }, (err, _doc) => {
            ...
            return this.coda(request, reply, err, _doc, data, ['database', 'update']);
        });
    }
    ...
    delete(request, reply) {
        ...
        this.Collection.findOneAndRemove(query, (err, doc) => {
            ...
            return this.coda(request, reply, err, result, data, ['database', 'delete']);
        });
    }

    coda(request, reply, err, result, data, _tags = []) {

        const resultIsArray = (result && result.constructor === Array);
        let tags = _tags;
        let normalizedResult = (resultIsArray && result.length === 1) ? result[0] : result;
        let response = JSend.success(normalizedResult); // HTTP 201
        ...
        if (result && (resultIsArray) && !result.length) {
